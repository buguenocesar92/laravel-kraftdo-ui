...
@once
    <style>
        .muni-acc { border:1px solid var(--kd-border); border-radius:var(--kd-radius); overflow:hidden; background:var(--kd-surface); }
        .muni-acc__item + .muni-acc__item { border-top:1px solid var(--kd-border); }
        .muni-acc__head { display:flex; align-items:center; justify-content:space-between; gap:12px; width:100%; padding:15px 18px; background:transparent; border:none; cursor:pointer; font-family:var(--kd-font-sans); font-size:14px; font-weight:600; color:var(--kd-text); text-align:left; transition:background var(--kd-dur) var(--kd-ease); }
        .muni-acc__head:hover { background:var(--kd-surface-2); }
        .muni-acc__head:focus-visible { outline:none; box-shadow:inset var(--kd-ring); }
        .muni-acc__chevron { color:var(--kd-muted); transition:transform var(--kd-dur) var(--kd-ease); display:inline-flex; }
        .muni-acc__chevron--open { transform:rotate(180deg); }
        .muni-acc__body { padding:0 18px 16px; font-family:var(--kd-font-sans); font-size:13.5px; color:var(--kd-muted); line-height:1.6; }
        .muni-acc-enter { transition:opacity var(--kd-dur) var(--kd-ease), transform var(--kd-dur) var(--kd-ease); }
