...
<head>
    ...
    <style>
        *,*::before,*::after{ box-sizing:border-box; }
        ...
        .muni-auth-aside{ display:none; position:relative; overflow:hidden; padding:48px; flex-direction:column; justify-content:space-between; background:var(--kd-surface); border-right:1px solid var(--kd-border); }
    ...
    <aside class="muni-auth-aside">
        <div style="position:relative;display:flex;align-items:center;gap:11px;">
            <div style="height:40px;display:flex;align-items:center;padding:5px 12px;background:#fff;border-radius:var(--kd-radius-sm);">
                @if ($logo){{ $logo }}@else<span style="font-family:var(--kd-font-mono);font-weight:700;color:#0b0f14;">GRA</span>@endif
            </div>
            <div><div style="font-weight:700;font-size:14px;">{{ $system }}</div><div style="font-size:12px;color:var(--kd-muted);">Ecosistema municipal</div></div>
        </div>
        <div style="position:relative;">
            ...
            @unless (isset($aside))
                ...
                <p style="font-size:13.5px;color:var(--kd-muted);line-height:1.6;max-width:36ch;margin-top:12px;">Tus datos están protegidos bajo la Ley 21.719 y no salen del municipio.</p>
            @endunless
        </div>
