...
@php
    ...
    $bg = $tone === 'accent' ? 'var(--kd-accent-soft)' : 'var(--kd-surface-3)';
    $fg = $tone === 'accent' ? 'var(--kd-accent)' : 'var(--kd-text)';
@endphp
...
<span
    {{ $attributes->merge([
        'style' => "display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;"
            ...
            ."font-family:var(--kd-font-sans);font-size:{$font}px;font-weight:700;"
            .($src ? '' : "background:{$bg};color:{$fg};border:1px solid var(--kd-border);"),
    ]) }}
