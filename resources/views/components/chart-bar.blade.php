...
<div {{ $attributes->merge(['class' => 'kd-chartbar']) }} style="display:flex;flex-direction:column;">
    ...
    @if ($labels)
        <div style="display:flex;gap:8px;margin-top:8px;">
            @foreach ($norm as $d)
                <span style="flex:1;text-align:center;font-family:var(--kd-font-sans);font-size:10.5px;color:var(--kd-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;">{{ $d['label'] }}</span>
            @endforeach
        </div>
    @endif
</div>
