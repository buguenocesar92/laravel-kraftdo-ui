...
<div style="overflow-x:auto;border:1px solid var(--kd-border);border-radius:var(--kd-radius);background:var(--kd-surface);">
    <table {{ $attributes->merge(['style' => 'width:100%;border-collapse:collapse;font-family:var(--kd-font-sans);font-size:12.5px;']) }}>
        @if (! empty($columns))
            <thead>
                <tr>
                    @foreach ($columns as $col)
                        <th style="text-align:left;white-space:nowrap;padding:9px 12px;font-family:var(--kd-font-sans);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.03em;color:var(--kd-muted);background:var(--kd-surface-2);border-bottom:1px solid var(--kd-border);">{{ $col }}</th>
                    @endforeach
                </tr>
            </thead>
        @endif
        <tbody>
            ...
            @else
                <tr>
                    <td colspan="{{ max(count($columns), 1) }}" style="text-align:center;padding:28px 12px;color:var(--kd-muted);">{{ $empty }}</td>
                </tr>
            @endif
        </tbody>
    </table>
