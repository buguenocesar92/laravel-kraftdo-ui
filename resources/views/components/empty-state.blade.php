...
<div {{ $attributes->merge(['style' => 'display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:10px;padding:48px 24px;']) }}>
    <div aria-hidden="true" style="display:grid;place-items:center;width:52px;height:52px;border-radius:14px;background:var(--kd-surface-2);border:1px solid var(--kd-border);color:var(--kd-muted);">
    ...
    <div style="font-family:var(--kd-font-sans);font-size:15px;font-weight:700;color:var(--kd-text);">{{ $title }}</div>
    @if ($description)
        <p style="margin:0;max-width:44ch;font-size:13px;color:var(--kd-muted);line-height:1.5;">{{ $description }}</p>
    @endif
