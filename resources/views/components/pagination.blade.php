...
<nav aria-label="Paginación" {{ $attributes->merge(['style' => 'display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:16px;font-family:var(--kd-font-sans);font-size:13px;']) }}>
    <a href="{{ $current > 1 ? $link($current - 1) : '#' }}" @if($current <= 1) aria-disabled="true" @endif
       class="kd-page kd-page--nav" style="{{ $current <= 1 ? 'opacity:.4;pointer-events:none;' : '' }}">‹ Anterior</a>

    @php $prev = 0; @endphp
    @foreach ($pages as $p)
        @if ($p - $prev > 1)<span style="color:var(--kd-hint);padding:0 2px;">…</span>@endif
        @if ($p === $current)
            <span class="kd-page kd-page--current" aria-current="page">{{ $p }}</span>
        @else
            <a href="{{ $link($p) }}" class="kd-page">{{ $p }}</a>
        @endif
        @php $prev = $p; @endphp
    @endforeach

    <a href="{{ $current < $total ? $link($current + 1) : '#' }}" @if($current >= $total) aria-disabled="true" @endif
       class="kd-page kd-page--nav" style="{{ $current >= $total ? 'opacity:.4;pointer-events:none;' : '' }}">Siguiente ›</a>
...
@once
    <style>
        .kd-page { display:inline-flex;align-items:center;justify-content:center;min-width:32px;height:32px;padding:0 9px;
        ...
        .kd-page:hover { background:var(--kd-surface-2);color:var(--kd-text); }
        .kd-page:focus-visible { outline:none;box-shadow:var(--kd-ring); }
        .kd-page--current { background:var(--kd-accent);color:var(--kd-on-accent);font-weight:600; }
        .kd-page--nav { color:var(--kd-text);font-weight:500; }
    </style>
@endonce
