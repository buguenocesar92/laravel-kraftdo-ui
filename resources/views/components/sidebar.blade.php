...
{{-- Barra lateral de navegación para dashboards. En móvil se colapsa (toggle con el
     evento `muni-sidebar`). El slot son <x-kd::nav-item> y <x-kd::nav-section>. --}}
<aside
...
@once
    <style>
        .muni-sb { flex-shrink:0; width:var(--sb-w); background:var(--kd-surface); border-right:1px solid var(--kd-border); }
        ...
        @media (max-width:899px) {
            .muni-sb { position:fixed; inset:0 auto 0 0; z-index:150; transform:translateX(-100%); transition:transform var(--kd-dur) var(--kd-ease); box-shadow:var(--kd-shadow-lg); }
