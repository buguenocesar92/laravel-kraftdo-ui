...
{{-- Pestañas (Alpine 3). `tabs` es un array de etiquetas; los paneles van en el slot como
     <x-kd::tab-panel> en el mismo orden. Navegación con flechas ←/→. --}}
<div x-data="{ active: {{ (int) $default }}, count: {{ count($tabs) }} }" {{ $attributes }}>
    <div
        role="tablist"
        style="display:flex;gap:2px;border-bottom:1px solid var(--kd-border);margin-bottom:16px;overflow-x:auto;"
